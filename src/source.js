...
async function main() {
    const { env } = process, ringApi = new RingApi({
        refreshToken: env.RING_REFRESH_TOKEN,
        debug: false,
    }), locations = await ringApi.getLocations(), allCameras = await ringApi.getCameras();
    ...
    if (allCameras.length) {
        allCameras.forEach(async (camera) => {
            camera.onNewNotification.subscribe(async (notification) => {
                const action = notification.android_config.category, event = action === PushNotificationAction.Motion
                    ? 'Motion detected'
